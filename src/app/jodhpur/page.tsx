...
export default function JodhpurPortalPage() {
  const {
    documents,
    entities,
    events,
  ...
  const crossAgencyLinks = relationships.filter((r) => r.source_document_ids.some((id) => jodhpurDocs.some((jd) => jd.id === id)));
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans">
      ...
      {/* Jodhpur Agency Portal Header Banner */}
      <div className="bg-cyan-950/40 border-b border-cyan-500/30 px-4 py-3">
        <div className="max-w-[1920px] mx-auto flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="text-xs font-mono uppercase font-bold tracking-wider px-2 py-0.5 rounded bg-cyan-900 text-cyan-200 border border-cyan-700">
                  Terminal 01 • JPD-CRIME
                </span>
                <span className="text-xs font-mono text-cyan-400">Commissionerate West</span>
              </div>
              <h2 className="text-lg font-black text-white tracking-tight">
                Jodhpur Police Investigative Terminal
              </h2>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4 text-xs font-mono">
            <div className="bg-slate-900/90 border border-slate-800 rounded-lg px-3 py-1.5">
              <span className="text-slate-400 block text-[10px]">VAULT EVIDENCE</span>
              <span className="text-cyan-400 font-bold text-sm">{jodhpurDocs.length} Files</span>
            </div>
            <div className="bg-slate-900/90 border border-slate-800 rounded-lg px-3 py-1.5">
              <span className="text-slate-400 block text-[10px]">CORRIDOR LEADS</span>
              <span className="text-amber-400 font-bold text-sm">{crossAgencyLinks.length} Cross-District</span>
            </div>
            <button
              onClick={() => setShowUpload(true)}
              className="px-3.5 py-2 bg-cyan-600 hover:bg-cyan-500 text-white rounded-lg font-semibold flex items-center gap-2 shadow-lg shadow-cyan-600/30 transition"
            >
      ...
      <main className="flex-1 max-w-[1920px] w-full mx-auto p-3 grid grid-cols-1 lg:grid-cols-12 gap-3 overflow-hidden">
        {/* Left Column: Jodhpur Evidence Vault (3 cols) */}
        <div className="lg:col-span-3 flex flex-col gap-3 h-[calc(100vh-175px)]">
          <div className="bg-slate-900/90 border border-slate-800 rounded-lg p-3 flex items-center justify-between">
            <div className="flex items-center gap-2 text-xs font-mono font-bold text-slate-200">
              ...
              <span>Jodhpur Evidence Vault ({jodhpurDocs.length})</span>
            </div>
            <span className="text-[10px] font-mono text-cyan-400">Classified Chain</span>
          </div>
          ...
          <div className="flex-1 overflow-y-auto space-y-2.5 pr-1">
            {jodhpurDocs.map((doc) => (
              <div
                ...
                className="p-3 bg-slate-900/70 border border-slate-800 hover:border-cyan-500/60 rounded-lg cursor-pointer transition group"
              >
        ...
        <div className="lg:col-span-5 flex flex-col gap-2 h-[calc(100vh-175px)]">
          ...
          <div className="flex items-center justify-between bg-slate-900/90 border border-slate-800 rounded-lg p-1">
            <div className="flex items-center gap-1">
              <button
                ...
                className={`px-3 py-1.5 rounded text-xs font-mono font-semibold transition ${
                  activeTab === 'graph' ? 'bg-cyan-600 text-white shadow' : 'text-slate-400 hover:text-white'
                }`}
              >
            ...
            <span className="text-[10px] font-mono text-slate-400 pr-2">
              Showing Jodhpur-linked cross connections
            </span>
          </div>
      ...
      {showUpload && (
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
          <EvidenceDropzone onClose={() => setShowUpload(false)} defaultAgencySlug="jodhpur" />
        </div>
      )}
    </div>
  );
